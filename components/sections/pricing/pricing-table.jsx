      ...
      <Container spacing={['verticalXXXLrg']}>
        ...
        <div className={css.currencySection}>
          <div className={css.currencyToggleContainer}>
            <span className={`${css.currencyLabel} ${currency === "USD" ? css.active : ''} ${currency === "USD" ? css.usd : ''}`}>
            ...
            <span className={`${css.currencyLabel} ${currency === "COP" ? css.active : ''} ${currency === "COP" ? css.cop : ''}`}>
          ...
          {currency === "COP" && (
            <div className={css.trmInfo}>
              <DollarSign />
              <span>
                {t('pricing.currency.trm')}: {loading ? <span className={`${css.skeleton} ${css.inline}`}></span> : `$${trm && trm.toLocaleString("es-CO")}`}
              </span>
            </div>
          )}
        </div>
              ...
              <div key={index} className={css.pricingCard}>
                ...
                <div className={css.cardContent}>
                  <div className={css.priceSection}>
                    <div className={css.priceRange}>
                      <span className={`${css.price} ${currency === "COP" ? css.medium : css.large}`}>
                        {loading ? <span className={`${css.skeleton} ${css.inline}`}></span> : formatPrice(item.minPrice)}
                      </span>
                      <span className={css.priceSeparator}>–</span>
                      <span className={`${css.price} ${currency === "COP" ? css.medium : css.large}`}>
                        {loading ? <span className={`${css.skeleton} ${css.inline}`}></span> : formatPrice(item.maxPrice)}
                      </span>
                    </div>
                  ...
                  <div className={css.featuresList}>
                    {t(`pricing.projects.${item.type}.features`, { returnObjects: true }).map((feature, featureIndex) => (
        ...
        <div className={css.disclaimerSection}>
          <div className={css.disclaimerBox}>
            <p className={css.disclaimerText}>
              <strong>{t('pricing.disclaimer.title')}</strong> {t('pricing.disclaimer.text')}
